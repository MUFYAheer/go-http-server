...
func TestLeague(t *testing.T) {
	...
	t.Run("it returns 200 status and league table as JSON on /league", func(t *testing.T) {
...
func getLeagueFromResponse(t testing.TB, rb io.Reader) []Player {
	t.Helper()
	var got []Player

	err := json.NewDecoder(rb).Decode(&got)
	if err != nil {
		t.Fatalf("Unable to parse response from server %q into slice of Player, '%v'", rb, err)
	}
